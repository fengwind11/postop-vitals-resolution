...
$FormalRoot = Split-Path -Parent $ScriptRoot
$OutputRoot = Join-Path $FormalRoot '02_data_internal'
$LogRoot = Join-Path $FormalRoot '10_logs'
...
function Invoke-ReadOnlyCsv {
    param([string]$Database,[string]$Name,[string]$Sql,[int]$TimeoutSeconds=900)
...
, candidate_stays AS (
 ...
 SELECT c.*,p.anchor_age+(extract(year from c.intime)::int-p.anchor_year) AS age_at_icu,p.gender,
...
$Queries = @(
    [pscustomobject]@{Database='mimiciv';Name='mimic_formal_cohort';Sql=$MimicCodeMap+$MimicCohort+@'
SELECT subject_id,hadm_id,stay_id,intime,outtime,age_at_icu,gender,alignment_tier,death28
FROM final ORDER BY stay_id
'@;Timeout=900},
    [pscustomobject]@{Database='mimiciv';Name='mimic_formal_counts';Sql=$MimicCodeMap+$MimicCohort+@'
SELECT count(*) AS n,count(DISTINCT subject_id) AS patients,sum(death28) AS deaths28,
       sum((gender IS NULL)::int) AS sex_missing,sum((age_at_icu IS NULL)::int) AS age_missing
FROM final
'@;Timeout=900},
    [pscustomobject]@{Database='sicdb';Name='sicdb_formal_cohort';Sql=$SicdbCohort+@'
SELECT f."CaseID" AS caseid,f."PatientID" AS patientid,f."ICUOffset" AS icuoffset,
       f."TimeOfStay" AS timeofstay,f."AgeOnAdmission" AS age_on_admission,
       f."Sex" AS sex_code,r."ReferenceValue" AS sex_label,f.death28
FROM final f
LEFT JOIN public.d_references r ON r."ReferenceGlobalID"=f."Sex"
ORDER BY f."CaseID"
'@;Timeout=900},
    [pscustomobject]@{Database='sicdb';Name='sicdb_formal_counts';Sql=$SicdbCohort+@'
SELECT count(*) AS n,count(DISTINCT "PatientID") AS patients,sum(death28) AS deaths28,
       sum(("Sex" IS NULL)::int) AS sex_missing,sum(("AgeOnAdmission" IS NULL)::int) AS age_missing
FROM final
'@;Timeout=900}
)
...
try {
    ...
    $Mimic = Import-Csv -LiteralPath (Join-Path $OutputRoot 'mimic_formal_cohort.csv')
    $Sicdb = Import-Csv -LiteralPath (Join-Path $OutputRoot 'sicdb_formal_cohort.csv')
    $MimicEvents = ($Mimic | Measure-Object -Property death28 -Sum).Sum
    $SicdbEvents = ($Sicdb | Measure-Object -Property death28 -Sum).Sum
    $MimicPatients = ($Mimic.subject_id | Sort-Object -Unique).Count
    $SicdbPatients = ($Sicdb.patientid | Sort-Object -Unique).Count
    if ($Mimic.Count -ne 1740 -or $MimicEvents -ne 168 -or $MimicPatients -ne 1740) {
        throw "STOP_AND_DEBUG_COHORT: MIMIC N=$($Mimic.Count), events=$MimicEvents, patients=$MimicPatients; expected 1740/168/1740"
    }
    if ($Sicdb.Count -ne 2376 -or $SicdbEvents -ne 185 -or $SicdbPatients -ne 2376) {
        throw "STOP_AND_DEBUG_COHORT: SICdb N=$($Sicdb.Count), events=$SicdbEvents, patients=$SicdbPatients; expected 2376/185/2376"
    }
    $Manifest = [pscustomobject]@{
        status='SUCCESS'
        authorization='FORMAL_MODELING'
        seed=20260918
        read_only=$true
        mimic_n=$Mimic.Count
        mimic_deaths28=[int]$MimicEvents
        mimic_unique_patients=$MimicPatients
        sicdb_n=$Sicdb.Count
        sicdb_deaths28=[int]$SicdbEvents
        sicdb_unique_patients=$SicdbPatients
        completed_at=(Get-Date).ToString('o')
        files=$Run
        script_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $MyInvocation.MyCommand.Path).Hash.ToLowerInvariant()
    }
    $Manifest | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $LogRoot 'formal_extraction_manifest.json') -Encoding utf8
